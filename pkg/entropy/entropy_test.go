...
func TestEntropyInString(t *testing.T) {
    ...
    // Fire
    response, err := FindHighEntropyWords(input, Base64CharsetName, 20, 4.5)

    require.NoError(t, err)
    require.NotNil(t, response)
    require.Len(t, response, 1)
    require.Equal(t, encoded, response[0].Value)
}
...
func TestEntropyWholeString(t *testing.T) {
    ...
    // Fire
    response, err := FindHighEntropyWords(encoded, Base64CharsetName, 20, 4.5)
