...
func (p *PEMParser) Parse(finding *database.Finding, findingString *database.FindingString) (parsedSecrets []*parsedSecret, err error) {
    ...
    if !p.diff.Line.IsAddOrDel && p.diff.Line.CodeEndsWith(header) && p.diff.NextLine().IsDel {
        ...
        p.diff.Increment()
        keyLines = []string{}
        p.diff.WhileTrueCollectCode(func(line *diffpkg.Line) bool { return line.IsDel }, &keyLines)
        key1 := &parsedSecret{
        ...
        p.diff.Increment()
        keyLines = []string{}
        p.diff.WhileTrueCollectCode(func(line *diffpkg.Line) bool { return line.IsAdd }, &keyLines)
        key2 := &parsedSecret{
    ...
    if p.diff.Line.IsAddOrDel && p.diff.Line.CodeEndsWith(header) {
        p.diff.Increment()
        keyLines = []string{}
        p.diff.UntilTrueCollectCode(func(line *diffpkg.Line) bool { return line.CodeStartsWith(footer) }, &keyLines)
